...
class Chat(models.Model):
    ...
    chat_id = models.CharField(max_length=255, verbose_name='Id группы', unique=True)
    name = models.CharField(max_length=255, verbose_name='Название группы')
    url_link = models.CharField(max_length=255, verbose_name='Ссылка на тп/магазин', choices=Const.LINK,
                                default='tp', help_text='Выбрать ссылку для перехода пользователя, после нажатия'
                                                        ' на клавишу "согласен"')
    # здесь language нужен для группового чата
    language = models.CharField(max_length=2, choices=Const.LANGUAGE, default='ru', verbose_name='Язык')
    url_chat = models.URLField(max_length=255, verbose_name='Ссылка на чат', blank=True, null=True,
...
class Users(models.Model):
    ...
    iduser = models.CharField(max_length=255, verbose_name='ID пользователя', db_index=True)
    username = models.CharField(max_length=255, verbose_name='Username пользователя', blank=True, null=True)
    fullname = models.CharField(max_length=255, verbose_name='Полное имя пользователя', blank=True, null=True)
    date_joined = models.DateTimeField(auto_now_add=True, verbose_name='Дата добавления пользователя')
    state = models.CharField(max_length=1, choices=Const.STATE, default='1', db_index=True)
    chat_id = models.ForeignKey(Chat, on_delete=models.SET_NULL, null=True, related_name='users')
...
class Data(SingletonModel):
    ...
    text_before_btn = MDTextField(verbose_name='Текст рядом с кнопками (русский)',
                                  default='Текст для приветствия новых пользователей в чате на русском языке')
    text_before_btn_en = MDTextField(verbose_name='Текст рядом с кнопками (english)',
    ...
    btn1 = models.CharField(max_length=255, verbose_name='Кнопка 1 (русский)', default='Согласен')
    btn1_en = models.CharField(max_length=255, verbose_name='Кнопка 1 (english)', default='Agree')
